...
(cffi:defcfun ("TF_StringEncode" %TF-STRINGENCODE) size-t
  (src :string) (src-len size-t) (dst :string) (dst-len size-t) (stat status))
...
;;                               size_t* dst_len, TF_Status* status);
(cffi:defcfun ("TF_StringDecode" %TF-STRINGDECODE) size-t
  (src :string) (src-len size-t) (dst :pointer) (dst-len :pointer) (stat status))

;; // Return the size in bytes required to encode a string `len` bytes long into a
;; // TF_STRING tensor.
;; extern size_t TF_StringEncodedSize(size_t len);
(cffi:defcfun ("TF_StringEncodedSize" %TF-STRINGENCODEDSIZE) size-t (len size-t))

;; // --------------------------------------------------------------------------
;; // TF_SessionOptions holds options that can be passed during session creation.
;; typedef struct TF_SessionOptions TF_SessionOptions;
(cffi:defctype sessionoptions :pointer)

;; // Return a new options object.
;; extern TF_SessionOptions* TF_NewSessionOptions();
(cffi:defcfun ("TF_NewSessionOptions" %TF-NEWSESSIONOPTIONS) sessionoptions)
...
(cffi:defcfun ("TF_SetTarget" %TF-SETTARGET) :void
  (options sessionoptions) (target :string))
...
(cffi:defcfun ("TF_SetConfig" %TF-SETCONFIG) :void
  (options sessionoptions) (proto :pointer) (proto-len size-t) (stat status))

;; // Destroy an options object.
;; extern void TF_DeleteSessionOptions(TF_SessionOptions*);
(cffi:defcfun ("TF_DeleteSessionOptions" %TF-DELETESESSIONOPTIONS) :void
  (opt sessionoptions))
...
;;                                    TF_Status* status);
(cffi:defcfun ("TF_GraphSetTensorShape" %TF-GRAPHSETTENSORSHAPE) :void
  (gr graph) (o output) (dims :pointer) (ndim :int) (stat status))
...
;;                                     TF_Status* status);
(cffi:defcfun ("TF_GraphGetTensorNumDims" %TF-GRAPHGETTENSORNUMDIMS) :int
  (gr graph) (o output) (stat status))
...
;;                                    TF_Status* status);
(cffi:defcfun ("TF_GraphGetTensorShape" %TF-GRAPHGETTENSORSHAPE) :void
  (gr graph) (o output) (dims :pointer) (ndim :int) (stat status))
...
(cffi:defcfun ("TF_NewOperation" %TF-NEWOPERATION) operationdescription
  (gr graph) (op-type :string) (oper-name :string))

;; // Specify the device for `desc`.  Defaults to empty, meaning unconstrained.
;; extern void TF_SetDevice(TF_OperationDescription* desc, const char* device);
(cffi:defcfun ("TF_SetDevice" %_TF-SETDEVICE) :void
  (desc operationdescription) (device :string))
...
(cffi:defcfun ("TF_AddInput" %TF-ADDINPUT) :void
  (desc operationdescription) (in input))

;; // For inputs that take a list of tensors.
;; // inputs must point to TF_Output[num_inputs].
;; extern void TF_AddInputList(TF_OperationDescription* desc,
;;                             const TF_Output* inputs, int num_inputs);
(cffi:defcfun ("TF_AddInputList" %TF-ADDINPUTLIST) :void
  (desc operationdescription) (inputs :pointer) (num-inputs :int))

;; // Call once per control input to `desc`.
;; extern void TF_AddControlInput(TF_OperationDescription* desc,
;;                                TF_Operation* input);
(cffi:defcfun ("TF_AddControlInput" %TF-ADDCONTROLINPUT) :void
  (desc operationdescription) (in :pointer))
...
(cffi:defcfun ("TF_ColocateWith" %TF-COLOCATEWITH) :void
  (desc operationdescription) (op :pointer))
...
;; extern void TF_ImportGraphDefOptionsAddInputMapping(TF_ImportGraphDefOptions* opts,
;;                                                     const char* src_name, int src_index,
;;                                                     TF_Output dst);
...
;; // into.
;; extern void TF_GraphImportGraphDefOptionsRemapControlDependency(TF_ImportGraphDefOptions* opts,
;;                                                                 const char* src_name,
;;                                                                 TF_Operation* dst);

;; // Cause the imported graph to have a control dependency on `oper`. `oper`
;; // should exist in the graph being imported into.
;; extern void TF_ImportGraphDefOptionsAddControlDependency(TF_ImportGraphDefOptions* opts,
;;                                                          TF_Operation* oper);

;; // Add an output in `graph_def` to be returned via the `return_outputs` output
;; // parameter of TF_GraphImportGraphDef(). If the output is remapped via an input
;; // mapping, the corresponding existing tensor in `graph` will be returned.
;; extern void TF_ImportGraphDefOptionsAddReturnOutput(TF_ImportGraphDefOptions* opts,
...
;; extern void TF_GraphImportGraphDefWithReturnOutputs(TF_Graph* graph,
...
;; TF_Session* TF_LoadSessionFromSavedModel(const TF_SessionOptions* session_options,
;;                                          const TF_Buffer* run_options,
;;                                          const char* export_dir, const char* const* tags,
;;                                          int tags_len,
